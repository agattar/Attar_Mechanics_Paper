...
def Crosslinker(chnum, prob,minbond,maxbond):
    ...
    file = open("data.ex1","r")
    lines = file.readlines()
    file.close()
    
    b_list = np.zeros((chnum*4001+10000,2))
    count = 0
    ...
    lams = np.array(l_list); lams_f = lams[:,:6].astype(float)
    lams_f = lams_f[lams_f[:,0].argsort()]
    ...
    for n in range(len(dnaloc[:,:])):
        ...
        if r <= prob:
            neighbor_list = []
            Extracted = np.subtract(dnaloc_f[n,3:],lams_f[:,3:])
            Multiplied = np.multiply(Extracted,Extracted)
            Multiplied = np.sum(Multiplied,axis=1)  
            Distances = np.sqrt(Multiplied)
        
            Result = np.where((Distances[:] <= 1.50) & (Distances[:] >= 1.00))
    
            for k in Result[0]:
                neighbor_list.append(k)
            print("Completion Percentage",(n/len(dnaloc)*100))
            while (int(bondnumbers[n,1])) > 0:   
                
                if len(neighbor_list) != 0:
                    chosen = random.choice(neighbor_list)
                    if ([int(beadname[n][0]),int(lams[chosen][0])]==b_list[:,None]).all(-1).any() == False and ([int(lams[chosen][0]),int(beadname[n][0])]==b_list[:,None]).all(-1).any() == False:
                        bondnumbers[int(n),1] -= 1
                        ind += 1
                        bnum.append(str(ind)+" "+"3"+" "+str(beadname[n][0])+" "+str(int(lams_f[chosen][0]))+"\n")
                        b_list[count+b_index,0] = str(beadname[n][0]);b_list[count+b_index,1] = str(int(lams_f[chosen][0]))
    ...
    indstr = str(str(ind)+" "+"bonds"+"\n")
    lines[4] = indstr
    
    bnum.append("\n")
    lines.insert(bondline,bnum)
    
    fl = open("data.tethered",'w+')
